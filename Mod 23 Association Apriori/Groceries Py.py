...
groceries = []
with open("C:\\Users\\gopal\\Documents\\360DigiTMG\\mod 23\\groceries.csv") as f:
    groceries = f.read()
...
groceries_series.columns = ["transactions"] #Give name to the variable

# creating a dummy columns for the each item in each transactions ... Using column names as item name
X = groceries_series['transactions'].str.join(sep = '*').str.get_dummies(sep = '*')
frequent_itemsets = apriori(X, min_support = 0.003,max_len = 5, use_colnames = True,verbose=1)
...
rules = association_rules(frequent_itemsets, metric = "lift", min_threshold = .05) #8812 Rules
rules.head(20)
rules.sort_values('lift', ascending = False).head(10)
rules.to_csv('groceries_Rules.csv')
...
rules_no_redudancy.to_csv("grocerieswithout.csv")
os.getcwd()
